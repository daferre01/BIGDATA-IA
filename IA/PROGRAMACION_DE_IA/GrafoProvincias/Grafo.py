...
class Grafo:
  ...
  def pop_abiertos(self, modo):
    ...
    if modo == "profundidad":
      ...
      idx_min = 0
      for i, n in enumerate(self.abiertos):
        if (self.get_node_attribute(n, "distanciaOrg")+self.get_node_attribute(n, "distanciaDst")/2) < (self.get_node_attribute(self.abiertos[idx_min], "distanciaOrg")+self.get_node_attribute(self.abiertos[idx_min], "distanciaDst")/2):
          idx_min = i
      ret= self.abiertos.pop(idx_min)
    return ret
...
# excepto en el inicial que es 0
    for n in self.nodos:
      ...
      self.set_node_attributes(nodo_inicial, distanciaOrg=0, distanciaDst=0)
...
g.recorre_grafo(nodo_inicial = "A Coruña", nodo_destino = "Tarragona", modo = "A*")
g.dibuja()
ruta = g.genera_ruta("Tarragona")
g.dibuja_ruta(ruta)
print(ruta[::-1])
plt.show()
